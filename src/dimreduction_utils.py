"""
Title: Dimensionality Reduction Classes
Author: nehabinish
Development Date: 02/10/2024 | 13:19:00
Updated: 04/10/2024

Description: Utility script with different classes for dimensionality reduction methods done on Context IV dataset.

    1. PcaAnalysis:
        Class that handles Principal Component Analysis (PCA) on neural data for dimensionality reduction.
    2. FactorAnalysis:    
        Class that performs Factor Analysis (FA) using an Expectation-Maximization (EM) algorithm 
        on covariance matrices to identify underlying factors in the data.

"""

# System Imports & append path
import sys
sys.path.append('/Users/nehabinish/Desktop/ContextIV_paper/ContextIV_codes/src/')
...
class PcaAnalysis:
    ...
    Description:
    The PcaAnalysis class applies Principal Component Analysis (PCA) on neural data 
    for dimensionality reduction and visualization.

    Methods:
        - __init__: Initializes the class with the dataset and optional training/testing sets for cross-validation.
    """     


    def __init__(self, X, **kwargs):
        """
        Initializes the PcaAnalysis class with input data and optional cross-validation splits.

        Args:
            - X (np.ndarray): The dataset to be used for PCA. It should be a 2D array where rows are 
            ...
            - **kwargs: Optional keyword arguments for cross-validation and other parameters.
                ...
                - train (np.ndarray): Training dataset used in cross-validation. Should be provided if 'crossval' is True.
                - test (np.ndarray): Test dataset used in cross-validation. Should be provided if 'crossval' is True.
        """
...
class FactorAnalysis:
    ...
    Description:
    This class performs Factor Analysis (FA) using an Expectation-Maximization (EM) algorithm 
    on covariance matrices. FA is useful for discovering latent variables that explain 
    the covariance in observed data.

    Most functions in this class are adapted from the matlab codes for 
    "Cortical areas interact through a communication subspace", Semedo et al. (Neuron, 2019). 
    v0.2b MATLAB codes from Joao Semedo - fa_util
    ...
    Methods:
        - __init__: Initializes the class with the dataset, latent dimensionality `q`, 
                    and optional cross-validation splits.
        - factor_analysis: Performs Factor Analysis using the EM algorithm, 
                           with options for probabilistic PCA (PPCA) and FA methods.
        - logdet: Computes the logarithm of the determinant of a positive-definite matrix.
        - mvn_log_likelihood: Computes the log-likelihood of the data under a multivariate Gaussian distribution.
        - factor_analysis_test_log_likelihood: Computes the log-likelihood of test data under Factor Analysis models.
        - CrossValFa: Performs k-fold cross-validation for Factor Analysis and computes the log-likelihood 
                      for each latent dimension.
        - FactorAnalysisModelSelect: Selects the optimal latent dimensionality based on cross-validation loss values.
    """


    def __init__(self, X, q, **kwargs):
        """
        Initializes the FactorAnalysis class with input data and optional cross-validation splits.

        Args:
            - X (np.ndarray): The dataset to be used for factor analysis. 
                              It should be a 2D array where rows are samples and columns are features.
            - **kwargs: Optional keyword arguments for cross-validation and other parameters.
    ...
    def CrossValFa(self, cvNumFolds=10, parallel=True):
        ...
        if qMax == 0:
            ...
            explained_variance = d / np.sum(d)  # Fraction of explained variance for each 
